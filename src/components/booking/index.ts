/**
 * Booking Components Index
 *
 * This file exports all the booking-related components to make imports cleaner.
 */

export { PassengerLuggageForm } from "./PassengerLuggageForm";
export { PersonalDetailsForm } from "./personal-details-form";
export { default as VehicleSelection } from "./vehicle-selection";
export { BookingSummary } from "./booking-summary";
export { BookingForm } from "./BookingForm";
export type { VehicleOption, FareResponse } from "./common/types";
export { BookingProgress } from "./booking-progress";
export { default as VehicleSelectionContainer } from "./VehicleSelectionContainer";
