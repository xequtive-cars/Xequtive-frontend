...
import HourlyBookingForm from "@/components/hourly-booking/HourlyBookingForm";
import { PersonalDetailsForm as HourlyPersonalDetailsForm } from "@/components/hourly-booking/hourly-personal-details-form";
import HourlyVehicleSelection from "@/components/hourly-booking/hourly-vehicle-selection";
...
export default function HourlyBookingPage() {
  ...
  // New state variables for hourly booking
  ...
  const handleCalculateFare = async () => {
    try {
      ...
      if (bookingType === 'one-way') {
        ...
        fareRequestData = {
          ...
          returnDetails: {
            ...
            outboundStops: additionalStops.length > 0 ? additionalStops.map(stop => ({
              lat: stop.latitude,
              lng: stop.longitude,
            })) : undefined,
      ...
      if (response && response.fare && response.fare.vehicleOptions && response.fare.vehicleOptions.length > 0) {
        ...
        setFetchError("No vehicles available for this route");
        toast({
          title: "Fare calculation failed",
          description: "No vehicles available for this route",
          variant: "destructive",
        });
      }
    } catch (error) {
        ...
        bookingData = {
          ...
          returnDetails: {
            ...
            returnDateTime: {
              ...
              time: validateTime(returnTime)
            },
            returnStops: additionalStops.map(stop => ({
              address: stop.address || "",
              coordinates: {
                lat: stop.latitude,
                lng: stop.longitude
                      ...
                      <CardContent className="p-4 space-y-4">
                        <div className="flex justify-between items-center mb-2">
                          <h2 className="text-base font-semibold">
                            Journey Details - {bookingType === 'one-way' ? 'One-Way' : bookingType === 'hourly' ? 'Hourly' : 'Return'}
                          </h2>
                        </div>
                    ...
                    <Card className="border shadow-sm">
                      <CardContent className="px-4 space-y-2 py-0 my-0">
                        ...
                        <div>
                          ...
                          <div className="p-2 bg-muted/40 rounded-md text-sm">
                            {bookingType === 'one-way' ? 'One-Way Transfer' : 
                             bookingType === 'hourly' ? `Hourly (${hours} hours)` : 
                             'Return Journey'}
                          </div>
                        </div>
                        ...
                        <div>
                          <label className="text-sm font-medium mb-1 block text-muted-foreground">
                            {bookingType === 'return' ? 'Dropoff Location (Outbound)' : 'Dropoff Location'}
                          </label>
                          <div className="p-2 bg-muted/40 rounded-md text-sm">
                            {dropoffLocation?.address || "Not specified"}
                          </div>
                        ...
                        {/* Return dropoff for return bookings */}
                        {bookingType === 'return' && (
                          <div>
                            <label className="text-sm font-medium mb-1 block text-muted-foreground">
                              Return Location
                            ...
                            <div className="p-2 bg-muted/40 rounded-md text-sm">
                              {pickupLocation?.address || "Same as pickup"}
                            </div>
                          </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-3">
                          <div>
                            <label className="text-xs font-medium mb-1 block text-muted-foreground">
                              {bookingType === 'return' ? 'Outbound Date' : 'Date'}
                            </label>
                            <p className="text-sm">
                              {selectedDate
                                ? new Date(selectedDate).toLocaleDateString(
                          ...
                          <div>
                            <label className="text-xs font-medium mb-1 block text-muted-foreground">
                              {bookingType === 'return' ? 'Outbound Time' : 'Time'}
                            </label>
                            <p className="text-sm">
                              {selectedTime || "Not selected"}
                            </p>
                        ...
                        <div className="mb-3">
                          ...
                          <div className="p-2 bg-muted/40 rounded-md text-sm">
                            ...
                            {/* Distance and Duration for One-Way and Return */}
                            {(bookingType === 'one-way' || bookingType === 'return') && fareData?.fare?.vehicleOptions?.[0]?.price?.messages && (
                              <div className="mb-2">
                                {fareData.fare.vehicleOptions[0].price.messages
                                  .filter((message: string) => message.includes('Distance:') || message.includes('Duration:'))
                    ...
                    <Card className="border shadow-sm sticky top-0 flex-1 flex flex-col justify-start">
                      <CardHeader className="py-0">
                        <CardTitle className="text-xl">
                          Booking Summary - {bookingType === 'one-way' ? 'One-Way' : bookingType === 'hourly' ? 'Hourly' : 'Return'}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="pt-0 space-y-4 flex-1 flex flex-col justify-start">
                        ...
                        {/* To field for One-Way and Return bookings */}
                        ...
                        <div>
                          <label className="text-sm font-medium mb-1 block text-muted-foreground">
                            {bookingType === 'return' ? 'Outbound Date & Time' : 'Date & Time'}
                          </label>
                          <div className="p-2 bg-muted/40 rounded-md text-sm">
                            {selectedDate
                          ...
                          <div className="mt-auto pt-4 border-t">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                <label className="text-sm font-medium text-muted-foreground">
                                  {bookingType === 'hourly' ? 'Total Price' : 'Base Price'}
                                </label>
                                <p className="text-2xl font-bold font-mono mt-1">
                                  {bookingType === 'hourly' ? (
                                    `£${(selectedVehicle.price.amount * hours * multipleVehicles).toFixed(2)}`
                                  ) : bookingType === 'return' ? (
                                    `£${(selectedVehicle.price.amount * 2).toFixed(2)}`
                                  ) : (
                                ...
                                {bookingType === 'hourly' && multipleVehicles > 1 && (
                                  <p className="text-xs text-muted-foreground mt-1">
                                    ({multipleVehicles} vehicles)
                                  </p>
                                )}
                              </div>
                            </div>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription className="text-base">
                ...
                processed. We have sent a booking confirmation to your email.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-md p-4 mb-4">
                ...
                <p className="text-sm text-slate-700 mt-2 font-semibold">
                  ...
                  >
                    ...
                    WhatsApp: (Urgent Support & Bookings within 24-hours)
                  </a>
                </p>
              </div>
